...
Fixpoint bin_to_nat (b : bin) : nat :=
  match b with
    | Z => O
    | N b' => S (2 * (bin_to_nat b'))
    | E b' => 2 * (bin_to_nat b')
  end.

Example test_bin_incr1 : incr (N Z) = E (N Z).
Proof. simpl. trivial. Qed.
Example test_bin_incr2 : incr (E (N Z)) = N (N Z).
Proof. simpl. trivial. Qed.
Example test_bin_incr3 : incr (N (N Z)) = E (E (N Z)).
Proof. simpl. trivial. Qed.
Example test_bin_incr4 : (bin_to_nat (N (E (N Z)))) + 1 = bin_to_nat (incr (N (E (N Z)))).
Proof. simpl. trivial. Qed.
Example test_bin_incr5 : bin_to_nat (N (E (E (N Z)))) = 9.
Proof. simpl. trivial. Qed.
(** [] *)
