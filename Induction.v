...
Proof.
  intros n m. induction n as [| n'].
  Case "n = 0".
    rewrite -> plus_O_n. rewrite -> plus_0_r.
    trivial.
  Case "n = S n'".
    rewrite <- plus_n_Sm.
    simpl.
...
induction is more powerful, durrrrr.
...
Proof.
  ...
  Case "n = 0".
    ...
    reflexivity.
  Case "n = S n'".
  assert (H: forall n : nat, evenb n = evenb (S (S n))).
...
(* prediction: just simpl/rewrite will suffice *)
(* actual: shit i'm stupid, obviously this requires induction...
 * it's on natural numbers not booleans, oops *)
...
(* gonna guess this can be proven using just simpl/rewrite *)
...
(* wait I thought I proved this myself lolol
 * err... destruct? I feel like simpl would suffice for andb_false_l but not this one... *)
...
(* gonna guess induction. I don't think it's worth trying anything less so I won't. *)
...
(* gonna guess simpl too but have a suspicion that won't be enough *)
...
Proof.
  intros b.
  induction b as [| half | minus_one_half].
  Case "b = Z".
    reflexivity.
  Case "b = E half".
    simpl.
    reflexivity.
  Case "b = N minus_one_half".
    simpl.
    rewrite -> IHminus_one_half.
    simpl.
    rewrite plus_0_r.
    rewrite plus_n_Sm.
    rewrite plus_n_Sm.
...
Theorem nat_to_bin_to_nat_correct : forall n : nat,
...
(* b. it's not true because binary representations aren't unique, I guess?
 * e.g. zero, twice zero, twice (twice zero), ... *)

Fixpoint normalize (b : bin) : bin :=
  match b with
    | N b' => N (normalize b')
    | E b' => match normalize b' with
                | Z => Z
                | b'' => E b''
              end
    | Z => Z
...
Theorem bin_to_nat_to_bin_normalized : forall b : bin,
    nat_to_bin (bin_to_nat b) = normalize b.
Proof.
  intros b.
  induction b as [| half | moh].
  Case "b = Z".
    reflexivity.
  Case "b = 2n".
